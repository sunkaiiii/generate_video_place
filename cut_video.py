...
def read_dir_video(path, mode=3, num=9999,generate_color=False,generate_sence=False):
    """
    如果要批量切割视频，应调用此方法
    :param path: 路径名
    ...
    for file in list:
        ...
        if not os.path.isdir(file):
            ...
            generate_video_info(cut_path,generate_color,generate_sence)
        count += 1

def read_video(filename, mode=3,generate_color=False,generate_sence=False):
    cut_path=cut_video(filename,mode)
    generate_video_info(cut_path,generate_color,generate_sence)

def generate_video_info(cut_path,generate_color=False,generate_scence=False):
    if generate_color==True:
        gc.generate_cut_video_color(cut_path)
    if generate_scence==True:
        gs.read_video_list(cut_path)
